...
urlpatterns = patterns('comments.views',
    ...
    url(r'^new/(\d+)/$',                'comment.new_comment',          name='new-comment'),
    url(r'^reply/(?P<parent_pk>\d+)/$', 'comment.new_comment',          name='comment-reply'),
    url(r'^new_comment/(?P<content_type>[\w.]+)/(?P<object_pk>\d+)/$',
                                        'comment.new_comment',      name='comment-toplevel-reply'),
